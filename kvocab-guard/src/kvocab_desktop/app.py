...
def _run_smoke_analyze() -> None:
    from kvocab_core.config import LEVEL_META, cover_image_path
    ...
    missing_covers = [
        ...
        if (cover := cover_image_path(level)) is None or not cover.exists()
    ]
    if missing_covers:
        raise RuntimeError(f"Bundled cover smoke test failed: missing {missing_covers}")
